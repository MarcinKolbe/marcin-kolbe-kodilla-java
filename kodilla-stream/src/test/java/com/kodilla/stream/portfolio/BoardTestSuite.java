...
public class BoardTestSuite {
    ...
    void testAddTaskListAverageWorkingOnTask() {
        ...
        List<TaskList> inProgressTasks = new ArrayList<>();
        inProgressTasks.add(new TaskList("In progress"));
        var daysSum = project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
                .flatMap(tl -> tl.getTasks().stream())
                .map(Task::getCreated)
                .mapToLong(d -> Period.between(d, LocalDate.now()).getDays())
                .sum();
        var taskCount = project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
                .flatMap(tl -> tl.getTasks().stream())
                .count();
        var averageTaskDuration = project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
                .flatMap(tl -> tl.getTasks().stream())
                .map(Task::getCreated)
        ...
        assertEquals(10.0, averageTaskDuration);
    }
